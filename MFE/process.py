...
def process_raw_data(dataset_path, processed_file, protein_list):
    ...
    train_len = int(len(G_list)*0.2)
    val_len = int(len(G_list)*0.1)
    test_len = int(len(G_list) * 0.9)
    G_list_train = G_list[:train_len]
    G_list_val = G_list[train_len:train_len+val_len]
    G_list_test = G_list[test_len:]
    # 存储数据为pkl文件
    with open(processed_file, 'wb') as f:
        pickle.dump(G_list_train, f)
    f.close()
    processed_file_val = processed_file.replace('train','valid')
    with open(processed_file_val, 'wb') as f:
        pickle.dump(G_list_val, f)
    f.close()
    processed_file_test = processed_file_val.replace('valid', 'test')
    with open(processed_file_test, 'wb') as f:
        pickle.dump(G_list_test, f)
    f.close()
    # 直接处理数据
