...
from MFE.dmasif_encoder.helper import diagonal_ranges
from MFE.dmasif_encoder.benchmark_models import dMaSIFConv_seg
...
class Atom_embedding_MP(nn.Module):
    ...
    def forward(self, x, y, y_atomtypes, x_batch, y_batch):
        ...
        for i in range(self.n_layers):
            ...
            messages = self.mlp[i](features.float())  # N,8,6
...
class dMaSIF(nn.Module):
    ...
    def features(self, P):
        ...
        chemfeats = self.atomnet(
            P["xyz"], P["atoms_xyz"], P["atomtypes"], P["batch"], P["batch_atoms"]
        )
    ...
    def forward(self, P):
        ...
        features = self.dropout(self.features(P)).float()
