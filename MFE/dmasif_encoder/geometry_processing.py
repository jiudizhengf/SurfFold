...
from MFE.dmasif_encoder.helper import *
...
# def assign_curvature_to_alphaC(
#         surface_points: torch.Tensor,  # shape=(len1,3)
#         curvature: torch.Tensor,        # shape=(len1,10)
#         alpha_coords: torch.Tensor,     # shape=(len2,3)
#         aggregation: str = "mean"       # 聚合方式: "mean", "sum", "max"
#     ) -> torch.Tensor:
#         """
#         将表面点的曲率数据按最近alphaC原子聚合,返回形状为 (len2,10) 的Tensor。
#         """
#         # Step 1: 确定每个表面点对应的alphaC索引（使用KDTree）
#         alpha_tree = KDTree(alpha_coords.cpu().numpy())
#         _, nearest_alpha_indices = alpha_tree.query(surface_points.cpu().numpy(), k=1)
#         nearest_alpha_indices = torch.tensor(nearest_alpha_indices,device=surface_points.device,dtype=torch.long).squeeze(-1)  # 压缩最后一个维度 (N,1) => (N,)
#         # 检查索引是否越界
#         invalid_indices = (nearest_alpha_indices < 0) | (nearest_alpha_indices >= len(alpha_coords))
#         print(torch.any(invalid_indices))  # 若输出True，则存在无效索引
#         # Step 2: 初始化结果Tensor
#         aggregated_curvature = torch.zeros((alpha_coords.shape[0], curvature.shape[1]), 
#                                 device=curvature.device)
        
#         # Step 3: 聚合曲率数据（避免Python循环，使用GPU加速）
#         if aggregation == "sum":
#             aggregated_curvature.index_add_(0, nearest_alpha_indices, curvature)
#         elif aggregation == "mean":
#             counts = torch.zeros((alpha_coords.shape[0], 1), device=curvature.device)
#             counts.index_add_(0, nearest_alpha_indices, torch.ones_like(curvature[:, :1]))
#             aggregated_curvature.index_add_(0, nearest_alpha_indices, curvature)
#             aggregated_curvature = aggregated_curvature / counts.clamp(min=1)  # 避免除零
#         elif aggregation == "max":
#             # 使用scatter_reduce实现最大值聚合
#             aggregated_curvature.scatter_reduce_(
#                 0, 
#                 nearest_alpha_indices[:, None].expand(-1, curvature.shape[1]), 
#                 curvature, 
#                 reduce="amax", 
#                 include_self=False
#             )
...
def soft_distances(x, y, batch_x, batch_y, num_atoms, smoothness=0.01, atomtypes=None):
    ...
    if atomtypes is not None:
        atomic_radii = torch.cuda.FloatTensor(
            [155,170,152,180], device=x.device,
        )
        # atomic_radii = atomic_radii.to(dtype=torch.float64)
        # atomic_radii = torch.cuda.FloatTensor(
        #     [120, 182, 170, 155, 152,
        #      227, 173, 180, 180, 275,
        #      231, 161, 156, 152, 126,
        ...
        )  # n_atoms, 1 数量级（0.7,0.7,.....,0.5）
        smoothness_i = LazyTensor(smoothness[:, None, None])

        mean_smoothness = (-D_ij.sqrt()).exp().sum(0) # Tensor([[0.7539],[0.0457],[0.0819],...,[0.3048],[0.1163],[0.1774]])
...
def atoms_to_points_normals(
    ...
    with torch.enable_grad():
        ...
        for it in range(nits):
            # tensor([1.1889, 2.1765, 1.9364,  ..., 1.4178, 1.4759, 2.0457], device='cuda:0'）
...
class dMaSIFConv(nn.Module):
    ...
    def forward(self, points, nuv, features, ranges=None):
        ...
        for head in range(self.n_heads):
            ...
            # Features:
            f_j = LazyTensor(head_features[None, :, :].float())  # (1, N, Hd)
            ...
            a_1 = LazyTensor(A_1.view(1, 1, -1).to(torch.float32))  # (1, 1, C*3)
            b_1 = LazyTensor(B_1.view(1, 1, -1).to(torch.float32))  # (1, 1, C)
            a_2 = LazyTensor(A_2.view(1, 1, -1).to(torch.float32))  # (1, 1, Hd*C)
            b_2 = LazyTensor(B_2.view(1, 1, -1).to(torch.float32))  # (1, 1, Hd)
        ...
        features = torch.cat(head_out_features, dim=1).float()  # n_heads * (N, Hd) -> (N, H)
